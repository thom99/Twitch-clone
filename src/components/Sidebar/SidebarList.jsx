        ...
        <>
            {
                dati.slice(0, visible).map(el => {
                    return (
                        <Link key={el.user_id} to={`/livestream/${el.user_id}`} className={classes.linkStream}>
                            <SidebarElement img={el.profile_image_url} name={el.user_name} game={el.game_name} viewer={el.viewer_count} offline={el.offline}/>
                        </Link>
                    )
                })
            }
